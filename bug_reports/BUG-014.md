...
- **Implemented by**: W-1765141289-59911
- **Commit**: 450d7edb703a94d7d81dbb1b0193aa802a8c7743
- **Regression test file**: `tests/embedding/test_bug_014_mps_workaround.py`

### Changes Made

**1. Modified `src/learning_memory_server/embedding/nomic.py`:**
   - Added `import torch` to access PyTorch device APIs
   - Added MPS workaround in `NomicEmbedding.__init__` after model load
   - Forces model to CPU when MPS is available to avoid memory leak
   - Added detailed comments explaining the workaround

**2. Created regression test `tests/embedding/test_bug_014_mps_workaround.py`:**
   - `test_bug_014_embedding_model_uses_cpu_on_mps()`: Verifies model device is CPU on Apple Silicon
   - `test_bug_014_large_indexing_completes()`: Indexes 50+ files to verify no memory issues
   - `test_bug_014_batch_encoding_performance()`: Tests batch encoding for progressive slowdown

All changes follow the fix plan exactly. The implementation is minimal and targeted.
