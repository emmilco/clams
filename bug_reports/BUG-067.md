...
Change `register_mcp_server()` and `merge_mcp_server()` to register an SSE-type server instead of command-based, using `settings.server_host` and `settings.server_port` to construct the URL. Update all existing tests to match the new format. Details below.
