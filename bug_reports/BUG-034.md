...
- **Implemented by**: W-1765438452-45878
- **Commit**: 211282f
- **Regression test file**: tests/storage/test_bug_034_regression.py

### Changes Made

Implemented exactly as planned:

1. **src/clams/storage/qdrant.py** (lines 59 and 65):
   - Removed `int()` cast from timeout parameter
   - Added `# type: ignore[arg-type]` comments for mypy compatibility
   - Float timeouts are now passed directly to AsyncQdrantClient

2. **tests/storage/test_bug_034_regression.py** (new file):
   - 4 regression tests verifying float timeouts are preserved:
     - `test_subsecond_timeout_not_truncated_to_zero`: Verifies 0.5 stays 0.5 (not 0)
     - `test_float_timeout_preserves_decimal`: Verifies 5.9 stays 5.9 (not 5)
     - `test_integer_timeout_still_works`: Verifies backward compatibility
     - `test_url_mode_timeout_not_truncated`: Verifies fix in URL mode (non-memory)
