...
from calm.embedding.registry import (
    EmbeddingRegistry,
    get_code_embedder,
...
def test_registry_not_initialized() -> None:
    ...
    import calm.embedding.registry as registry_module
...
def test_lazy_loading_code_embedder() -> None:
    ...
    # Verify model was loaded by checking dimension property
    assert embedder1.dimension == 384  # MiniLM-L6-v2 dimension
...
def test_lazy_loading_semantic_embedder() -> None:
    ...
    assert embedder1 is not None
    # Verify model was loaded by checking dimension property
    assert embedder1.dimension == 768  # Nomic embed dimension
...
def test_env_var_override_code_model() -> None:
    """Test that CALM_CODE_MODEL env var is read by CalmSettings."""
    custom_model = "custom/code/model"

    with patch.dict(os.environ, {"CALM_CODE_MODEL": custom_model}):
        # CalmSettings reads the env var with CALM_ prefix
        from calm.config import CalmSettings

        settings = CalmSettings()
        assert settings.code_model == custom_model

        # The registry uses the model name passed from CalmSettings
...
def test_env_var_override_semantic_model() -> None:
    """Test that CALM_SEMANTIC_MODEL env var is read by CalmSettings."""
    custom_model = "custom/semantic/model"

    with patch.dict(os.environ, {"CALM_SEMANTIC_MODEL": custom_model}):
        # CalmSettings reads the env var with CALM_ prefix
        from calm.config import CalmSettings
...
def test_registry_separate_instances() -> None:
    ...
    # Should have different dimensions (proves they're different models)
    assert code_embedder.dimension == 384  # MiniLM-L6-v2
    assert semantic_embedder.dimension == 768  # Nomic embed
