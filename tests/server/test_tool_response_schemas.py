...
from clams.server.tools.memory import VALID_CATEGORIES, get_memory_tools
...
@pytest.fixture
def search_tools(searcher: Searcher) -> dict[str, Any]:
    """Get search tools."""
...
def session_manager(temp_session_dirs: tuple[Path, Path]) -> SessionManager:
    ...
    return SessionManager(clams_dir=clams_dir, journal_dir=journal_dir)
...
@pytest.fixture
def context_tools(searcher: Searcher, value_store: ValueStore) -> dict[str, Any]:
    """Get context tools."""
    return get_context_tools(searcher, value_store)
...
class TestRetrieveMemoriesResponseSchema:
    ...
    @pytest.mark.asyncio
    async def test_success_response_structure(
        self, memory_tools: dict[str, Any], mock_services: ServiceContainer
    ) -> None:
