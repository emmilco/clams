...
async def test_index_codebase_not_available(mock_services):
    ...
    with pytest.raises(
        MCPError,
        match="Code indexing not available.*CodeIndexer.*SPEC-002-06",
    ):
        await index_codebase(directory="/tmp", project="test")
...
async def test_search_code_not_available(mock_services):
    ...
    with pytest.raises(
        MCPError,
        match="Code search not available.*CodeIndexer.*SPEC-002-06",
    ):
...
async def test_find_similar_code_not_available(mock_services):
    ...
    with pytest.raises(
        MCPError,
        match="Code similarity.*not available.*CodeIndexer.*SPEC-002-06",
    ):
        await find_similar_code(snippet="test code")
...
# Tests for when CodeIndexer IS available
# (will be relevant when SPEC-002-06 is complete)
