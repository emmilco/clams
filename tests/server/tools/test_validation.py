...
Tests cover all helper functions in src/clams/server/tools/validation.py:
...
class TestValidateTags:
    ...
    def test_tags_at_max_length(self) -> None:
        """Tags with exactly 50 chars should pass."""
        validate_tags(["x" * 50])  # Should not raise
    ...
    def test_tag_too_long_shows_index(self) -> None:
        """Error message should show tag index."""
        with pytest.raises(ValidationError) as exc_info:
            validate_tags(["short", "x" * 60])
...
class TestValidateLanguage:
    ...
    def test_valid_language_mixed_case(self) -> None:
        """Mixed case valid language should pass."""
        validate_language("Python")  # Should not raise

    def test_valid_language_uppercase(self) -> None:
        """Uppercase valid language should pass."""
        validate_language("TYPESCRIPT")  # Should not raise
...
class TestValidatePositiveInt:
    ...
    def test_valid_positive(self) -> None:
        ...
        validate_positive_int(100, "value")  # Should not raise
    ...
    def test_at_max_val(self) -> None:
        """Value at max_val should pass."""
        validate_positive_int(100, "value", max_val=100)  # Should not raise

    def test_above_max_val_raises(self) -> None:
        """Value above max_val should raise ValidationError."""
        with pytest.raises(ValidationError, match="exceeds maximum"):
            validate_positive_int(101, "value", max_val=100)

    def test_param_name_in_error(self) -> None:
        """Parameter name should appear in error message."""
        with pytest.raises(ValidationError) as exc_info:
...
class TestValidateTextLength:
    ...
    def test_none_allowed(self) -> None:
        """None text should pass."""
        validate_text_length(None, max_length=100, param_name="note")

    def test_valid_within_limit(self) -> None:
        """Text within limit should pass."""
        validate_text_length("short", max_length=100, param_name="note")

    def test_valid_at_limit(self) -> None:
        """Text at exactly the limit should pass."""
        validate_text_length("x" * 100, max_length=100, param_name="note")

    def test_exceeds_limit_raises(self) -> None:
        """Text exceeding limit should raise ValidationError."""
        with pytest.raises(ValidationError, match="too long"):
            validate_text_length("x" * 101, max_length=100, param_name="note")

    def test_error_shows_length(self) -> None:
        """Error message should show actual length."""
        with pytest.raises(ValidationError) as exc_info:
            validate_text_length("x" * 150, max_length=100, param_name="note")
        assert "150" in str(exc_info.value)
        assert "100" in str(exc_info.value)

    def test_empty_allowed_by_default(self) -> None:
        """Empty string should pass by default."""
        validate_text_length("", max_length=100, param_name="note")

    def test_empty_raises_when_not_allowed(self) -> None:
        """Empty string should raise when allow_empty=False."""
        with pytest.raises(ValidationError, match="cannot be empty"):
            validate_text_length("", max_length=100, param_name="note", allow_empty=False)

    def test_whitespace_raises_when_not_allowed(self) -> None:
        """Whitespace-only string should raise when allow_empty=False."""
        with pytest.raises(ValidationError, match="cannot be empty"):
            validate_text_length(
                "   ", max_length=100, param_name="note", allow_empty=False
            )
