...
import calm.tools.memory as memory_module
...
@pytest.fixture
def mock_services(fresh_vector_store: AsyncMock, mock_embedder: MagicMock) -> dict:
    """Create mock services dict with fresh vector store."""
    return {
...
class TestBug043CommitsCollection:
    ...
    async def test_index_commits_creates_collection(
        ...
        from calm.git import GitAnalyzer
...
class TestBug043ValuesCollection:
    ...
    async def test_store_value_creates_collection(
        ...
        from calm.values import ClusterInfo, ValueStore
