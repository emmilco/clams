...
def validate_text_length(
    text: str | None, max_length: int = 2000, param_name: str = "text"
) -> None:
    """Validate text length."""
    if text is not None and len(text) > max_length:
        raise ValidationError(
            f"Field '{param_name}' exceeds {max_length} character limit "
            f"({len(text)} chars)"
        )
...
def get_ghap_tools(
    ...
    async def update_ghap(
        ...
        try:
            ...
            error_type = (
                "validation_error" if isinstance(e, ValidationError) else "not_found"
            )
    ...
    async def resolve_ghap(
        ...
        try:
            ...
            if status == "falsified":
                ...
                if not category:
                    raise ValidationError("Field 'root_cause.category' is required")
                if not description:
                    raise ValidationError("Field 'root_cause.description' is required")
